...
local function syndromeIsDfHackSyndrome(syndrome)
    for k,v in ipairs(syndrome.syn_class) do
        if v.value:find('DFHACK_') then return true end
    end
    return false
end
...
local function itemHasSyndrome(item)
    ...
    for _,material in ipairs(itemSyndromeMats) do
        for __,syndrome in ipairs(material.syndrome) do
            if syndrome.syn_name == item.subtype.name then 
				table.insert(allItemSyndromes,syndrome)
			end
        end
    end
...
local function eraseSyndrome(target,syn_id)
    for i=#target.syndromes.active-1,0,-1 do
        if target.syndromes.active[i].type==syn_id then 
			for _,symptom in ipairs(target.syndromes.active[i].symptoms) do
				symptom.flags = 3
			end		
		end
    end
...
local function itemIsInValidPosition(item_inv, syndrome)
    ...
    return not ((item_inv.mode == 0 and not item.AffectsHauler) 
				or (item_inv.mode == 7 and not item.AffectsStuckins) 
				or (item_inv.mode ~= 2 and item.IsArmorOnly) 
				or (item_inv.mode ~=1 and item.IsWieldedOnly) 
				or (item_inv.mode ~=7 and item.OnlyAffectsStuckins))
end
...
local function addOrRemoveSyndromeDepending(unit,item,old_equip,new_equip,syndrome)
	local item_inv = itemInventorySlot(unit,item)
	if syndromeIsDfHackSyndrome(syndrome) and creatureIsAffected(unit,syndrome) then
		if itemIsInValidPosition(item_inv, syndrome) then
...
eventful.onInventoryChange.itemsyndrome=function(unit_id,item_id,old_equip,new_equip)
    ...
    if itemMaterial then
        for k,syndrome in ipairs(itemMaterial.syndrome) do
            ...
            addOrRemoveSyndromeDepending(unit,item,old_equip,new_equip,syndrome)
        end
    end
