...
--[[
	...
	Usage : fovcommand <source_unit> <radius> <command> [<target_raw>] [<arg>]
	...
	- Radius : The lenght of the radius to cast around the source unit.
...
-- Checking args
if not args[3] then qerror('Please enter the command you wish to run on the target units') end
...
if args[4] then target_raw = args[4] end
...
for i = #unitList - 1, 0, -1 do
	...
	if isSelected(unitTarget, view) then
		...
		if args[5] then
			dfhack.run_script(args[3], unitTarget.id, args[4])
		else
			dfhack.run_script(args[3], unitTarget.id)
		end
